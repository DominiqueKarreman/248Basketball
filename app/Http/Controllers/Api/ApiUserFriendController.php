...
class ApiUserFriendController extends Controller
{
    ...
    public function index()
    {
        ...
        $user = Auth()->user();
        // dd($user);
        $friends = UserFriend::where('user_id', $user->id)->where('is_mutual', 1)->join('users', 'user_friends.friends_id', '=', 'users.id')->select('user_friends.*', 'users.name', 'users.online', 'users.profile_photo_path', 'user_friends.id as id')->get();
